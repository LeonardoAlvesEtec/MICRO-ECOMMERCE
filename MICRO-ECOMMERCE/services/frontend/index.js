...
function calculateShipping(id) {
    fetch('http://localhost:3000/shipping' + cep) //Faz requisição para a API
        .then((data) => {
            if (data.ok) {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://localhost:3000/products')
        ...
        .then((data) => {
            if (data) {
                //Para cada livro, cria e adiciona o elemtno ao container
                data.forEach((book) => {
            ...
            document.querySelectorAll('.button-shipping').forEach((btn) => {
                btn.addEventListener('click', (e) => {
                    const id = e.target.getAttribute('data-id'); //Pega ID do livro
                    const cep = document.querySelector(`.book[data-id="${id}"] input`).value; //Pega o CEP digitado
                    calculateShipping(id, cep); //Chamaa função de frete
                });
            });


            //Adicionaa evento de clique aos botões de compra
